...
const Home = () => {
  ...
  const xapContractAddressBsc = "0x1b3781eA482113695622EF6C31Fb38c4e02C95A0";
  ...
  const [xapContractBsc, setxapContractBsc] = useState(null);
  ...
  const updateEthers = () => {
    ...
    let xapContractBsc = new ethers.Contract(
      xapContractAddressBsc,
      xap_abi,
      tempSigner
    );
    ...
    setxapContractBsc(xapContractBsc);
  };
  ...
  const borrowHandler = async (e) => {
    ...
    let ParsedborrowAmount = ethers.utils.parseEther(borrowAmount);
    let borrowtxn = await contract.borrow(ParsedborrowAmount);
  ...
  const sendTokenHandler = async (e) => {
    ...
    let parsedqty = ethers.utils.parseEther(qty);
  ...
  const approveTestEth = async (e) => {
    e.preventDefault();
    await testEthContract.approve(
      contractAddress,
      "9999999999999999999999999999999999999999999999999999999999999999"
    );
  };

  const TausdBalanceHandlerBsc = async () => {
    let balanceTausdToken = await xapContractBsc.balanceOf(defaultAccount);
    let tokenBalance = ethers.utils.formatEther(balanceTausdToken);
  ...
  const testColleteralHandler = async () => {
    ...
    let tokenBalance = ethers.utils.formatEther(balanceTestToken);
  ...
  return (
    <div className="bg-gray-900 h-screen overflow-y-auto">
      <div className="flex justify-between">
        ...
        <h1 className="text-gray-500 text-3xl font-bold">Astria Finance MVP</h1>
        <button
      ...
      <div className="bg-gray-600 rounded-xl h-3/4 mx-3 my-3 p-3 md:h-1/2 overflow-y-auto">
        <h2 className="text-white mt-3 font-bold ">Choose Chain</h2>
        ...
        <div>
          ...
          <div className="bg-gray-500 p-3 rounded-md">
            <form onSubmit={mintHandler}>
              ...
              />
              <button className="bg-gradient-to-r from-cyan-500 to-blue-500 rounded w-32">
                mint
              </button>
            </form>
          </div>
          ...
          <div className="bg-gray-500 p-3 rounded-md">
            <form onSubmit={depositHandler}>
              <div className="text-white font-thin">
                TestCollateral {TestBalance}
              </div>
              <input
                id="depositAmount"
                type="string"
                placeholder="0.0"
                className="w-full bg-gray-500 focus:bg-gray-500 rounded"
              />

              <button
                onClick={approveTestEth}
                className="bg-gradient-to-r from-cyan-500 to-blue-500 rounded w-32 mr-5"
              >
                Approve
              </button>
              <button className="bg-gradient-to-r from-cyan-500 to-blue-500 rounded w-32">
        ...
        <p className="text-red-500 ">
          Hint: To test out this mvp, The first step is mint. Mint issues or
          transfers a token called “testCollateral”. This “testColleteral” token
          is currently the only accepted collateral on the astria mvp.
        </p>
      </div>
      <div className="bg-gray-600 rounded-xl h-3/4 mx-3 my-5 p-3 overflow-y-auto md:h-1/2">
        ...
        <div>
          ...
          <div className="bg-gray-500 p-3 rounded-md">
            <form onSubmit={borrowHandler}>
              <div className="text-white font-thin">TAUSD {TausdBalance}</div>
              <input
                id="borrowAmount"
                type="string"
                placeholder="0.0"
