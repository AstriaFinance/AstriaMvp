...
const Home = () => {
  ...
  const TausdbalanceHandler = async () => {
    let balanceTausdToken = await xapContractAddress.balanceOf(defaultAccount);
    let balanceTausdNum = balanceTausdToken.toNumber();
    let decimals = await xapContractAddress.getDecimals();
    let tokenBalance = balanceTausdNum / Math.pow(10, decimals);
    setTausdBalance(tokenBalance);
  };

  const testColleteralHandler = async () => {
    let balanceTestToken = await testEthContractAddress.balanceOf(
      defaultAccount
    );
    let balanceTestTokenNum = balanceTestToken.toNumber();
    let decimals = await testEthContractAddress.getDecimals();
    let tokenBalance = balanceTestTokenNum / Math.pow(10, decimals);

    setTestBalance(tokenBalance);
  };
  useEffect(() => {
    if (contract != null) {
    }
  ...
  return (
    <div>
      <div className="flex bg-black">
        <div className="rounded bg-white mr-10">
          <div className="mx-2">TestCollteral</div>
          <div className="mx-2">{TestBalance}</div>
        </div>
        <div className="rounded bg-white m2-2">
          <div className="mx-2">TAUSD</div>
          <div className="mx-2">{TausdBalance}</div>
        </div>
        <div className="rounded bg-white ml-10 ">
          <div className="mx-5">Wallet</div>
          <div className="w-32 text-sm truncate hover:text-clip">
            {defaultAccount}
          </div>
        </div>
      </div>
